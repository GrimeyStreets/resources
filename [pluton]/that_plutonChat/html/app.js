window.APP = {
	...
	data() {
		return {
			...
			removedSuggestions: [],
			templates: {
				'default': '<div class="chat-message">{0} says: {1}&lrm;</div>',
				'defaultAlt': '{0}',
				'print': '<pre>{0}</pre>',
	...
	mounted() {
		post('http://that_plutonChat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		ON_OPEN() {
			document.getElementsByClassName("chat-messages")[0].classList.remove("invisScroll");

			if (this.input == null){
				this.input = document.getElementById("t-input");
			}
			if (this.inputLengthSpan == null){
				this.inputLengthSpan = document.getElementById("inputLength");
			}

            this.inputLengthSpan.innerHTML = this.input.value.length + "/" + this.input.maxLength;
		...
		send(e) {
			...
			if(this.message !== '') {
				post('http://that_plutonChat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('http://that_plutonChat/chatResult', JSON.stringify({ canceled }));
			}else{
				this.message = '';
			}
			document.getElementsByClassName("chat-messages")[0].classList.add("invisScroll");
