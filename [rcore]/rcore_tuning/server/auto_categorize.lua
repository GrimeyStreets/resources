Citizen.CreateThread(function()
    ...
    if HasOkokVehicleshop() then
        AutoCategorizeOkokVehicleshop()
    end
end)
...
function HasOkokVehicleshop()
    return #MySQL.Sync.fetchAll('SHOW TABLES LIKE "okokvehicleshop_vehicles";') > 0
end

function AutoCategorizeOkokVehicleshop()
    local dbVehicles = MySQL.Sync.fetchAll([[
        SELECT vehicle_id as model, min_price as price
        FROM okokvehicleshop_vehicles
    ]])
