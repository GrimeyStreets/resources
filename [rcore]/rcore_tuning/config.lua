Config = {
	...
	Components = false,
	ComponentItemName = 'tuning_component',

	-- used to obtain components
	...
	MoneyCostMultiplier = 70, -- only used if Components = false
	ComponentMoneyMultiplier = 10,
	ComponentPerScrapMultiplier = 1, -- By default, it'll give between 1-5 scrap per box.

	-- if Components = false, mechanics will use money from society
	UseSocietyMoney = true,
	--[[
		If false, everyone can use tuning shops and jobs aren't used at all
	]]
	UseJobs = true,
	UpgradesPurchasable = true, -- If false, all stores upgrades are automatically unlocked.

	EnableCamberAndSpacers = true,

	EnableNitrous = true,
	EnableNitrousUI = true,
	...
	UseQbProgressbar = false,

	UseESXProgressbar = false,

	OnlyOwnersCanTuneOwnedCars = true,

	AllowBulletproofTires = true,
	AllowDriftTyres = true,
	
	AllowChameleonPaint = true,
	ChameleonPaintCostMultiplier = 10,

	BeeplessHelpText = true,
	TuningInstallDelayOnRetrieval = 0,

	Keys = {
        CANCEL = {code = 194, label = 'INPUT_FRONTEND_RIGHT'},

		MENU_MODIFIER = {code = 21, label = 'INPUT_SPRINT'},
		MENU_KEY = {code = 311, label = 'INPUT_REPLAY_SHOWHOTKEY'},

		-- use this if you want to use single key for tuning menu
		-- MENU_CATALOGUE_OPEN = {code = 170, label = 'INPUT_SAVE_REPLAY_CLIP'},
    },
	Text = {
        CANCEL = 'Cancel',
	},
	...
	DrillSoundRadius = 5.0, -- Drill sound when completing mechanic job will only play if players are this close
	DefaultCategory = 2, -- Set this to a number 0-4 (by default) to select a category by default
	BossMenuEnabled = false, -- If true, there will be another checkpoint to access the boss menu of the workplace. These coords can be found in TuningShops below

	ChargeOrderedPlayer = true, -- Won't work if UseSocietyMoney is true
	AddMarginToSociety = true, -- Works with ChargeOrderedPlayer to add the margin money to the society.
	DisregardZCoord = false, -- Useful for car lift scripts where car is significantly higher than player
	CheckWheelHealthForCamber = false, -- Set to true if vehicle's wheel health needs to be checked for compatibility with other scripts.
	AlignCatalogueRight = false,
	-- DisableApplyTuningOnVehicleEnter = true,
	RGBEnabled = true,
	ShowPricesToCustomers = true, -- If false, prices will not be shown to customers.
	DisableMarginChanges = false, -- Mechanics won't be able to change margins

	EnableAdminInstallCommand = true, -- Enables /installalltuning that will auto install all ordered tuning for that vehicle.
}
...
TuningShops = {
	LMESA = {
		...
		job = 'mechanic',
		minGrade = 0, -- Minimum grade that employee needs to install upgrades.


		interiors = {
			[153601] = true,
		},
		-- -- Use when shop doesn't register (says you're "You are not in your workshop")
		-- interiorless = {
		-- 	radius = 20.0,
		-- 	debug = false,
		-- },
		coords = vector3(732.59, -1078.7, 22.16),
		officeCoords = vector3(725.81, -1071.28, 27.26),
		bossMenuCoords = vector3(727.66, -1066.96, 27.26),
		paintBoothZone = { -- this is where mechanic has to put the car to be able to proceed with paint
			vector3(735.82, -1071.94, 21.79)
		},
		smokePos = {
			vector3(735.79, -1072.15, 22.23)
		},
		-- paintBoothZone = { -- Each paint booth zone must have a smokePos and paintBooth in stockpiles. The numbers must match. This is for multiple paint booths. (Coords are not setup properly.) -- this is where mechanic has to put the car to be able to proceed with paint
		-- 	[1] = vector3(-326.46, -144.9, 39.05), 
		-- 	[2] = vector3(-323.2081, -132.6628, 38.95931)
		-- },
		-- smokePos = {
		-- 	[1] = vector3(-326.98, -144.63, 40.36), 
		-- 	[2] = vector3(-323.2081, -132.6628, 38.95931)
		-- },
		...
		stockpiles = {
			bonnet = {
				vector3(737.39, -1066.43, 21.11),
			},
			exhaust = {
				vector3(737.52, -1081.89, 21.09),
			},
			paintBooth = { -- this triggers the paint process
				vector3(733.19, -1076.74, 21.12),
			},
			brakes = {
				vector3(733.99, -1064.3, 21.09),
			},
			paint = {
				vector3(737.75, -1078.14, 21.12),
			},
			wheels = {
				vector3(725.64, -1069.09, 21.11),
			},
		},
	...
	RANCHO = {
		...
		-- 	debug = false,
		-- },
		coords = vector3(479.9, -1316.5, 29.19), -- tp 479.9 -1316.5 29.19
		officeCoords = vector3(472.1, -1310.76, 28.14),
		bossMenuCoords = vector3(473.49, -1312.51, 28.14),
		paintBoothZone = { -- this is where mechanic has to put the car to be able to proceed with paint
			vector3(478.45, -1308.69, 29.22),
		},
		smokePos = {
			vector3(478.18, -1308.2, 29.23),
		},
        smokeRotation = vector3(0.0, 0.0, 0.0),
		modifier = 1.0,
		stockpiles = {
			bonnet = {
				vector3(475.55, -1319.55, 28.15),
			},
			exhaust = {
				vector3(483.15, -1325.35, 28.15),
			},
			paintBooth = { -- this triggers the paint process
				vector3(478.6, -1312.81, 28.13),
			},
			brakes = {
				vector3(473.63, -1316.23, 28.14),
			},
			paint = {
				vector3(475.52, -1310.32, 28.15),
			},
			wheels = {
				vector3(480.88, -1326.57, 28.15),
			},
		},
		disabledCategories = {
			-- ["cosmetics"] = true -- All category names can be found under Config.MenuOrder
	...
	Barney_Autos = {
		...
		coords = vector3(-211.92, -1324.53, 30.89),
		officeCoords = vector3(-197.43, -1360.45, 29.59),
		-- bossMenuCoords = vector3(-207.52, -1336.81, 34.89),
		paintBoothZone = { -- this is where mechanic has to put the car to be able to proceed with paint
			vector3(-195.79, -1343.24, 30.60),
		},
		smokePos = {
			vector3(-195.79, -1343.24, 30.60),
		},
		...
		stockpiles = {
			bonnet = {
				vector3(-195.80, -1316.95, 30.59),
			},
			exhaust = {
				vector3(-195.68, -1317.08, 30.59),
			},
			paintBooth = { -- this triggers the paint process
				vector3(-200.73, -1340.58, 30.59),
			},
			brakes = {
				vector3(-195.68, -1317.08, 30.59),
			},
			paint = {
				vector3(-215.63, -1316.44, 30.59),
			},
			wheels = {
				vector3(-205.74, -1316.37, 30.59),
			},
	...
	BURTON = {
		...
		-- 	debug = false,
		-- },
		coords = vector3(-336.33, -134.66, 39.0), -- tp -336.33 -134.66 39.0
		officeCoords = vector3(-345.27, -123.12, 37.93),
		bossMenuCoords = vector3(-342.94, -123.45, 37.93),
		paintBoothZone = { -- this is where mechanic has to put the car to be able to proceed with paint
			vector3(-326.46, -144.9, 39.05),
		},
		smokePos = {
			vector3(-326.98, -144.63, 40.36),
		},
        smokeRotation = vector3(0.0, 0.0, 0.0),
		modifier = 1.0,
		stockpiles = {
			bonnet = {
				vector3(-330.81, -126.99, 37.97),
			},
			exhaust = {
				vector3(-342.95, -140.33, 37.9),
			},
			paintBooth = { -- this triggers the paint process
				vector3(-329.97, -141.0, 37.95),
			},
			brakes = {
				vector3(-345.47, -131.41, 37.98),
			},
			paint = {
				vector3(-319.44, -131.99, 37.89),
			},
			wheels = {
				vector3(-337.37, -125.01, 37.95),
				vector3(-338.9, -133.8, 39.0),
			},
		},
		disabledCategories = {
			-- ["cosmetics"] = true -- All category names can be found under Config.MenuOrder
...
if Config.RGBEnabled then
	table.insert(
		ColorCategories,
		{name = 'rgb', label = _UWithFallback('rgb', "RGB")}
	)
end
...
function GetPlatesName(index)
	...
	elseif (index == 5) then
		return _UWithFallback("lp_yankton", "Yankton")
	elseif (index == 6) then
		return _UWithFallback("lp_ecola", "eCola")
	elseif (index == 7) then
		return _UWithFallback("lp_las_venturas", "Las Venturas")
	elseif (index == 8) then
		return _UWithFallback("lp_liberty_city", "Liberty City")
	elseif (index == 9) then
		return _UWithFallback("lp_lscm", "LS Car Meets")
	elseif (index == 10) then
		return _UWithFallback("lp_ls_panic", "LS Panic")
	elseif (index == 11) then
		return _UWithFallback("lp_ls_pounders", "LS Pounders")
	elseif (index == 12) then
		return _UWithFallback("lp_sprunk", "Sprunk")
	end
end
...
Config.Menus = {
	...
	modBrakes = {
		...
		upgrades = {
			...
			_UWithFallback("menu_upgrades_brakes_super", "Super Brakes"),
		},
		steps = {
			{
				bonePos = {'center'},
	...
	modArmor = {
		...
		isContinuous = true,
		upgrades = {
			[-1] = _UWithFallback("menu_upgrades_no_armor", "No armor"),
			[0] = _UWithFallback("menu_upgrades_armor_upgrade", "Armor Upgrade %s", "20%"),
			_UWithFallback("menu_upgrades_armor_upgrade", "Armor Upgrade %s", "40%"),
			_UWithFallback("menu_upgrades_armor_upgrade", "Armor Upgrade %s", "60%"),
			_UWithFallback("menu_upgrades_armor_upgrade", "Armor Upgrade %s", "80%"),
			_UWithFallback("menu_upgrades_armor_upgrade", "Armor Upgrade %s", "100%")
		},
		steps = {
			{
				bonePos = {'center'},
				label = _U("steps_armor_install"),
				helpLabel = _U("steps_armor_install_action"),
				time = math.random(10 * 1000, 15 * 1000),
				anim = {'mini@repair', 'fixing_a_player' , 1+16},
				onFinish = 'install',
			},
		...
		modifier = {[0] = 1.0, 2.0, 6.0, 8.0, 10.0, 12.0},
		bones = {'wheel_lf', 'wheel_rf', 'wheel_lr', 'wheel_rr', 'wheel_lm1', 'wheel_rm1', 'wheel_lm2', 'wheel_rm2', 'wheel_lm3', 'wheel_rm3'}
	},
	modTransmission = {
		...
		upgrades = {
			...
			_UWithFallback("menu_upgrades_transmission_super", "Super Transmission")
		},
		steps = {
			{
				bonePos = {'center'},
	...
	extras = {
		itemIndex = 'extras',
		label = _U("menu_extras"), --~REPLACE~'Změna barvy',
		parent = 'main',
	},
	modRoofLivery = {
	...
	modFrontWheelsTypes = {
		...
		modFrontWheelsTypeRear	= 'motorcycle_rear',
	...
	modFrontWheelsTypeRear = {
		itemIndex = 'modFrontWheelsTypeRear',
		label = _U("wheel_type_bike_rear"), --~REPLACE~'Motorka',
		parent = 'modFrontWheelsTypes',
		isBike = true,
		modType = 24,
		wheelType = 6,
...
ScrapyardConfig = {
	AnyVehicleAllowed = false,
	places = {
		{
			npcCoords = vector4(-420.7389, -1674.4390, 19.0290-0.98, 160.8332),
...
if Config.UseSocietyMoney and Config.ChargeOrderedPlayer then
	Config.ChargeOrderedPlayer = false
end

for i = 0, 50 do
	table.insert(WorkerMenuOrder, "extra_"..i)
	table.insert(Config.MenuOrder.extras, "extra_"..i)
	Config.Menus.cosmetics["extra_"..i] = "stickers"
	Config.Menus.extras["extra_"..i] = "extra_"..i
	Config.Menus["extra_"..i] = {
		itemIndex = "extra_"..i,
		label = _U("menu_cosmetics_extras"), --~REPLACE~'Polepy',
		parent = 'extras',
		price = 9.3,
		modType = "extra_"..i,
		steps = {
			{
				bonePos = {'center'},
				label = _U("steps_extra_install"),
				helpLabel = _U("steps_extra_install_action"),
				time = math.random(10 * 1000, 15 * 1000),
				anim = {'mini@repair', 'fixing_a_player' , 1+16},
				onFinish = 'install',
			},
...
if not Config.SocietyPrefix then
