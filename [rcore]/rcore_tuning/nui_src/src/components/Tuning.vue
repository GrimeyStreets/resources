<template>
    <div :class='{"tuning-menu": true, "left-side": side != "right", "right-side": side == "right"}'>
        <TuningTitle />
        <div class="tuning-categories">
            ...
            <CategoryList v-if='has3Levels && selectedLevel1 && selectedLevel2 && selectedLevel3' :grow='selectedLevel1 && selectedLevel2 && selectedLevel3'>
                ...
                <ColorPicker v-if="isColorTypeSelected && selectedLevel3 == `rgb`" 
                    :grow='selectedLevel1 && selectedLevel2 && !selectedLevel3' @select="previewTuningOption"
                    :type="selectedLevel2"/>
...
export default {
    ...
    computed: {
        isColorTypeSelected() {
            ...
            switch(this.selectedLevel2) {
                case 'primaryRespray':
                    return true;
                case 'secondaryRespray':
                    return true;
                default:
                    return false;
            }
    ...
    methods: {
        getInstalledWheelType() {
            const rearType = 'modFrontWheelsTypeRear';

            const isInstalled = (key) => {
                const installedValue = this.installedUpgrades[key];
                return installedValue !== null && installedValue !== undefined && installedValue >= 0;
            };

            if (this.selectedLevel3 === rearType) {
                if(isInstalled(rearType) === true) {
                    return rearType;
                }
            }

            for (let i = 0; i <= 12; i++) {
                const wheelTypeKey = `modFrontWheelsType${i}`;
                if (isInstalled(wheelTypeKey)) {
                    return wheelTypeKey;
                }
            }
        ...
        getOrderedWheelType() {
            ...
            const isValidOrder = (key) => {
                const orderedValue = this.orderedUpgrades[key];
                return orderedValue !== null && orderedValue !== undefined && orderedValue >= 0;
            };

            if (this.selectedLevel3 === rearType) {
                if(isValidOrder(rearType) === true) {
                    return rearType;
                }
            }
        ...
        selectLevel2: function(name) {
            ...
            if(this.selectedLevel3 == 'rgb' && !this.isColorTypeSelected) {
                this.selectedLevel3 = null;
            }
        },
        selectLevel3(name) {
