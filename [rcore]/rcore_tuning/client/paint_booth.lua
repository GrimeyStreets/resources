...
function GetMatchingSmokePos(coords, zone)
    if type(TuningShops[zone].smokePos) == 'vector3' then
        return TuningShops[zone].smokePos
    end

    local closestSmokePos = nil
    local closestDistance = nil

    for _, smokePos in pairs(TuningShops[zone].smokePos) do
        local distance = #(coords - smokePos)
        if (not closestSmokePos or not closestDistance) or (distance < closestDistance) then
            closestSmokePos = smokePos
            closestDistance = distance
        end
    end
    
    return closestSmokePos
end
...
function DoResprayEffect(zone, colorIdx)
    ...
    local smokePos = GetMatchingSmokePos(coords, zone)
    if #(coords - smokePos) > 50.0 then
        return
    end
    ...
    Citizen.CreateThread(function()
        ...
        for i = 1, 5 do
            ...
            PlaySoundFromCoord(soundId, 'SPRAY', smokePos.x, smokePos.y, smokePos.z, 'CARWASH_SOUNDS', 0, 10 , 0)
            RenderSpraySmoke(smokePos, doorData.smokeRotation, colorIdx)
            Citizen.Wait(1500)
            StopSound(soundId)
