...
function HandlePlateUpgrade(menuPtr)
    ...
    for i = 0, (gameBuild >= 3095 and 12 or 4) do
        table.insert(
            options,
            {
...
function HandleColorPicker(menuPtr, allowRgb)
    local options = {}


    for _, colData in pairs(ColorCategories) do
        local isUnlocked = (CurrentShop and PurchasedShopUpgrades[CurrentShop] and PurchasedShopUpgrades[CurrentShop]['paint_' .. colData.name]) or not CurrentShop 

        local colOptions = {}

        if colData.name ~= 'rgb' or allowRgb then
            if Colors[colData.name] then
...
function HandleModTireEnhancements(menuPtr)
    ...
    if Config.AllowDriftTyres ~= nil then
        driftTyresEnabled = Config.AllowDriftTyres
    end

    if driftTyresEnabled and gameBuild >= 2372 then
        table.insert(
            enh,
            {
...
function GetMaxModCount(menuPtr, modCount)
    if menuPtr.restrictUpgradeCount then
        maxModCount = menuPtr.restrictUpgradeCount
        return math.min(modCount, maxModCount)
    end
...
function HandleNumberedUpgrade(menuPtr)
    local modCount = GetNumVehicleMods(CurrentVehicle, menuPtr.modType) - 1
    local maxModCount = GetMaxModCount(menuPtr, modCount)
    local currentUpgradeIdx = TuningInstalled[MenuPosition]
    local currentOrderedIdx = TuningOrdered[MenuPosition]
    local options = {}
    for j = -1, math.min(4, maxModCount) do
        ...
        for i = 0, modCount do
            ...
            if modName then
                ...
                if modLabel == "NULL" then
                    modLabel = tostring(i+1).."/"..tostring(modCount)
                end
                ...
                table.insert(
                    options,
                    {
                        label = modLabel,
                        index = i,
                        price = GetNuiPrice(menuPtr.itemIndex, (not menuPtr.samePrice and i) or 0),
                    }
                )
            end
        end
...
function FilterTuningOptions(model, tuningOptions, depth)
    if depth == nil then
        depth = 0
    end

    if not Config.ModCombinationBlacklist then
        return tuningOptions
    end

    if not Config.ModCombinationBlacklist[model] then
        return tuningOptions
    end

    local returnItems = {}

    for key, tbl in pairs(tuningOptions) do
        if tbl.modName and Config.ModCombinationBlacklist[model][tbl.modName] then
        else
            if tbl.items then
                tbl.items = FilterTuningOptions(model, tbl.items, depth + 1)
            end

            table.insert(returnItems, tbl)
        end
    end
...
exports("OpenTuningPreview", function()
    OpenTuningMenu(true)
end)