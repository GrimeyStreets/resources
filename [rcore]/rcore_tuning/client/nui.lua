function GetTuningForNui(vehicle, preview)
    local tuningOptions = {}
    for _, catName in pairs(Config.MenuOrder.main) do
        local catItems = {}
        if (not CurrentShop or (CurrentShop and not TuningShops[CurrentShop].disabledCategories[catName])) then
            for _, modName in pairs(Config.MenuOrder[catName]) do
                if (not CurrentShop or (CurrentShop and not TuningShops[CurrentShop].disabledUpgrades[modName])) then
                    if modName == 'primaryRespray' or modName == 'secondaryRespray' or modName == 'pearlescentRespray' or modName == 'modFrontWheelsColor' or modName == 'interiorRespray' or modName == 'dashboardRespray' then                        
                        table.insert(
                            catItems,
                            {
                                ...
                                modName = modName,
                                items = HandleColorPicker(Config.Menus[modName], modName == 'primaryRespray' or modName == 'secondaryRespray'),
                            }
                        )
...
RegisterNUICallback('previewTuning', function(data, cb)
    ...
    if ((modName == "primaryRespray" or modName == "secondaryRespray") and type(index) == "table") then
    else
...
function SendEventToResetToInstalled()
    ...
    TriggerServerEvent("rcore_tuning:resetToInstalled", VehToNet(veh))
end
