	...
	while true do
        ...
        if WarMenu.IsMenuOpened('tuningWorkerMenu') then
            if not CurrentlyTuningVehNetId or not IsInsideShop or not IsControllingCurrentShop then
                ...
                if isVehDamaged then
                    if not TuningInProgress[CurrentlyTuningVehNetId] or (TuningInProgress[CurrentlyTuningVehNetId] and not (TuningInProgress[CurrentlyTuningVehNetId]['repair'] or TuningInProgress[CurrentlyTuningVehNetId]['motorBikesRepair'])) then
                        ...
                        if WarMenu.Button(_U("repair_vehicle"), FormatPrice(price), function() HoveringPart = 'repair' end) then
                            if isLightlyDamaged then
                                ...
                                if not Config.ChargeOrderedPlayer then
                                    ...
                                    local selectedPed = SelectPedToPay()
                                    if selectedPed then
                                        local selectedPlayerServerId = GetPlayerServerId(NetworkGetPlayerIndexFromPed(selectedPed))
                                        if OtherPlayerHasMoney(price, selectedPlayerServerId) then
                                            if GetVehicleType() == 8 then
                                                SelectCurrentInstallPart('motorBikesRepair', 1, selectedPlayerServerId)
                                            else
                                                SelectCurrentInstallPart('repair', 1, selectedPlayerServerId)
                                            end
                                        else
                                            ShowNotification(BuyTuningPriceErrorForPlayers(price))
                                        end
                                    end
                            ...
                            if display and not IsThisTuningInProgress(name) then
                                ...
                                if WarMenu.Button(GetTuningItemLabel(name, idx), FormatPrice(price), function() HoveringPart = name end) then
                                    if not Config.ChargeOrderedPlayer then
                                        ...
                                        if OtherPlayerHasMoney(price, ServerOrderedTuning[CurrentlyTuningVehNetId].orderedSrc) then
                                            SelectCurrentInstallPart(name, idx)
                                        else
                                            ShowNotification(BuyTuningPriceErrorForPlayers(price))
                                        end
                ...
                if Entity(NetToVeh(CurrentlyTuningVehNetId)).state.rcoreHasNitrous and Entity(NetToVeh(CurrentlyTuningVehNetId)).state.rcoreNitrous < 95 and not TuningShops[CurrentShop].disabledUpgrades['modNitrousRefill'] then
                    ...
                    if WarMenu.Button(_U('refill_nitrous'), FormatPrice(price), function() HoveringPart = 'repair' end) then
                        if Config.ChargeOrderedPlayer then
                            local selectedPed = SelectPedToPay()
                            if selectedPed then
                                local selectedPlayerServerId = GetPlayerServerId(NetworkGetPlayerIndexFromPed(selectedPed))
                                SelectCurrentInstallPart('modNitrousRefill', 1, selectedPlayerServerId)
                            end
                        else
...
Citizen.CreateThread(function()
    ...
    if Config.Keys.MENU_CATALOGUE_OPEN then
        AddTextEntry('RC_CANC_CATALOG', '~' .. Config.Keys.MENU_CATALOGUE_OPEN.label .. '~ '.._U("catalog_open")..'~n~~' .. Config.Keys.CANCEL.label .. '~ '.._U("catalog_tuning_cancel"))
        AddTextEntry('RC_CAT_PAS', '~' .. Config.Keys.MENU_CATALOGUE_OPEN.label .. '~ '.._U("catalog_show_to_customer"))
        AddTextEntry('RC_CAT_DRI', '~' .. Config.Keys.MENU_CATALOGUE_OPEN.label .. '~ '.._U("catalog_tuning_open"))
        AddTextEntry('RC_SHOW_ORDERED_TUNING', '~'.. Config.Keys.MENU_CATALOGUE_OPEN.label .. '~ '.._UWithFallback("catalog_tuning_open_ordered_tuning", "Open ordered tuning"))
    else
        ...
        AddTextEntry('RC_SHOW_ORDERED_TUNING', '~'.. Config.Keys.MENU_KEY.label .. '~ '.._UWithFallback("catalog_tuning_open_ordered_tuning", "Open ordered tuning"))
    end
    while not Config.TargetResourceName do
        if IsInsideShop and IsControllingCurrentShop then
            ...
            if not TemporarilySuspended then
                if not IsNearStageCheckpoint then
                    if isDriver then
                        if CurrentMinigamePart then
                            ExclusiveDisplayHelpTextThisFrame('RC_CANC_CATALOG')
                        else
                            ExclusiveDisplayHelpTextThisFrame('RC_CAT_DRI')
                        end
                    elseif isShotgun and realDriverPed > 0 then
                        ExclusiveDisplayHelpTextThisFrame('RC_CAT_PAS')
                    elseif not IsNearStageCheckpoint and not IsNearLift and CurrentMinigamePart then
                        ExclusiveDisplayHelpTextThisFrame('RC_CANC_TUN')
                    end
                end
                ...
                local isLJustPressed = false

                if Config.Keys.MENU_CATALOGUE_OPEN then
                    isShiftPressed = veh ~= 0
                    isLJustPressed = IsControlJustPressed(0, Config.Keys.MENU_CATALOGUE_OPEN.code) or IsDisabledControlJustPressed(0, Config.Keys.MENU_CATALOGUE_OPEN.code)
                else
                    isLJustPressed = IsControlJustPressed(0, Config.Keys.MENU_KEY.code) or IsDisabledControlJustPressed(0, Config.Keys.MENU_KEY.code)
                end
                ...
                if veh and veh > 0 and not CurrentMinigamePart then
                    ExclusiveDisplayHelpTextThisFrame('RC_SHOW_ORDERED_TUNING')
                end
                ...
                if isShiftPressed and isLJustPressed then
                    if IsBike(veh) or CurrentShop ~= 'EAST_V' then
                        if isDriver then
                            ...
                            if not IsVehicleDamaged(VehToNet(playerVehicle)) or Config.ForceVehicleRepair == false then
                                OpenTuningMenu()
                            else
                                ShowNotification(_U("catalog_vehicle_needs_repair"))
                            end
                        elseif isShotgun and realDriverPed > 0 and IsPedAPlayer(realDriverPed) then
                            if not IsVehicleDamaged(VehToNet(veh)) or Config.ForceVehicleRepair == false then
...
function DisplayTuningWorkerMenuForVehicle(veh, doNotUnsetMinigame)
    ...
    TriggerServerEvent('rcore_tuning:requestOpenWorkerMenu', vehNetId, doNotUnsetMinigame)
end

RegisterNetEvent('rcore_tuning:svOpenWorkerMenu', function(vehNetId, doNotUnsetMinigame)
    CurrentlyTuningVehNetId = vehNetId
...
function GetTuningVehicleInFront()
    ...
    if foundVehicle then
        if NetworkGetEntityIsNetworked(foundVehicle) then
            ...
            print("Error: Vehicle is not networked")
        end
    end
end
...
function GetTuningItemLabel(modName, idx, maybeVeh)
    ...
    if not menuPtr then
        print("ERROR: Missing menuPtr for", modName)
    end

    local base = menuPtr.label

    if not base then
        print("ERROR: Missing label for", modName)
    end

    if (modName == 'primaryRespray' or modName == 'secondaryRespray' or modName == 'pearlescentRespray' or modName == 'modFrontWheelsColor' or modName == 'interiorRespray' or modName == 'dashboardRespray') and type(idx) ~= "table" then
    ...
    elseif menuPtr.modType == 'modNitrous' then
        ...
        if type(idx) == "table" then
            if modName == "primaryRespray" then
                return "Custom Primary Spray"
            elseif modName == "secondaryRespray" then
                return "Custom Secondary Spray"
            end
        end
...
function GetMatchingStockpileCoords(modName, vehCoords)
    if type(TuningShops[CurrentShop].stockpiles[modName]) == 'vector3' then
        return TuningShops[CurrentShop].stockpiles[modName]
    end

    local closestStockpile = nil
    local closestDistance = nil

    for stockpileIndex, stockpileCoords in pairs(TuningShops[CurrentShop].stockpiles[modName]) do
        local distance = #(vehCoords - stockpileCoords)
        if (not closestStockpile or not closestDistance) or (distance < closestDistance) then
            closestStockpile = stockpileIndex
            closestDistance = distance
        end
    end

    return TuningShops[CurrentShop].stockpiles[modName][closestStockpile]
