...
Citizen.CreateThread(function()
    ...
    if not Config.DisableMarginChanges then
...
function HandleInOfficeCheckpoint()
    ExclusiveDisplayHelpTextThisFrame('TUN_OFFICE')
        ...
        while IsInOfficeCheckpoint do
            ...
            if WarMenu.IsMenuOpened('tuningOfficeMenu') then
                ...
                if IsControllingCurrentShop then
                    ...
                    if Config.UpgradesPurchasable and WarMenu.MenuButton(_U("menu_workshop_upgrade_buy"), 'tuningOfficeMenuUpgrades') then
                        OfficeMenuType = 'up'
                        SelectUpgradeItem('main')
                    elseif Config.UpgradesPurchasable and (DowngradeTimeout[CurrentShop] <= gameTime) and WarMenu.MenuButton(_U("menu_workshop_upgrade_sell"), 'tuningOfficeMenuUpgrades') then
                        OfficeMenuType = 'down'
                        SelectUpgradeItem('main')
                    elseif Config.UpgradesPurchasable and (DowngradeTimeout[CurrentShop] > gameTime) and WarMenu.Button(_U("workshop_upgrade_sell"), FormatTime(tonumber(math.ceil((DowngradeTimeout[CurrentShop] - gameTime)/1000)))) then
                    elseif WarMenu.MenuButton(_U("menu_workshop_margin"), 'tuningOfficeMenuMargin') then
                    end
                end

                WarMenu.Display()
            elseif WarMenu.IsMenuOpened('tuningOfficeMenuMargin') and not Config.DisableMarginChanges then
                local currentMargin = GlobalState['TUNING_MARGIN_' .. CurrentShop]
                if not currentMargin then
                    currentMargin = 0
                end
