...
function BuyTuningPriceErrorForPlayers(price)
    if Config.Components then
        return _UWithFallback("buy_tuning_price_error_components_other_player", "Player who ordered tuning doesn't have enough components for this tuning. It requires %s.", FormatPrice(price))
    else   
        return _UWithFallback("buy_tuning_price_error_money_other_player", "Player who ordered tuning doesn't have enough money for this tuning. It requires %s.", FormatPrice(price))
    end
end
...
function ComputeCurrentPrice(shop, modName, vehTuningModifier, idxToInstall, idxInstalled, vehicleModel, withMargin)
    ...
    if (modName == 'repair' or modName == 'motorBikesRepair') then
        toInstallCost = toInstallCost * (shop and ComputeRepairEfficacy(shop)) or 1.0
    end
    ...
    local baseCost = tonumber(math.ceil(finalBasePrice * (shop and TuningShops[shop].modifier or 1.0) * componentOrMoneyModifier))
    ...
    local currentMargin = shop and GlobalState['TUNING_MARGIN_' .. shop] or 0
    ...
    if type(idxToInstall) == "table" or idxToInstall < 1 then
        idxToInstall = 1
    end
...
--- fallback for new scrapyard
if not ScrapyardConfig and VehDisassemblyConfig then
    ScrapyardConfig = {
        AnyVehicleAllowed = false,
        places = {
            {
                npcCoords = vector4(
                    VehDisassemblyConfig.NPC.gps.x, 
                    VehDisassemblyConfig.NPC.gps.y,
                    VehDisassemblyConfig.NPC.gps.z,
                    VehDisassemblyConfig.NPC.heading
                ),
                npcModel = VehDisassemblyConfig.NPC.npcType,
                vehicleDelivery = VehDisassemblyConfig.vehicle.vehicleDelivery,
                deliveryPos = VehDisassemblyConfig.deliveryPos,
            },   
        },
        types = VehDisassemblyConfig.types,
    }
end

function isVehicleModelBlacklisted(model)
    if BlacklistedModels then
        for _, vehicleName in pairs(BlacklistedModels) do
...
function isVehiclePlateBlacklisted(vehPlate)
    if BlacklistedPlates then
        for _,  plate in pairs(BlacklistedPlates) do
